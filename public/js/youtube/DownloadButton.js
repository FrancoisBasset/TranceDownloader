...
export default class DownloadButton {
	static #element = document.getElementById('downloadButton');

	static {
		this.#element.onclick = this.#download;
	}

	static #download() {
		const url = VideosTable.getSelectedUrl();
		const artist = ArtistInput.getValue();
		const track = TrackInput.getValue();
